...
from torch import Tensor, cat, float32, randn, tensor, bfloat16
...
class ColorPaletteLatentDiffusionTrainer(LatentDiffusionTrainer[ColorPaletteLatentDiffusionConfig]):
    ...
    def compute_loss(self, batch: TextEmbeddingColorPaletteLatentsBatch) -> Tensor:
        ...
        # Used to run training on 2 parallel GPUs
        # self.unet.to(device=1)
        # clip_text_embedding.to(device=1)
        # noisy_latents.to(device=1)
        # noise.to(device=1)
    ...
    def compute_evaluation(self) -> None:
        ...
        for prompt in prompts:
            ...
            for i in range(num_images_per_prompt):
                ...
                clip_text_embedding = sd.compute_clip_text_embedding(text=prompt.text)
                cfg_color_palette_embedding = self.adapter.compute_color_palette_embedding(
                    tensor([prompt.color_palette])
                )
                
                self.adapter.set_color_palette_embedding(cfg_color_palette_embedding)
