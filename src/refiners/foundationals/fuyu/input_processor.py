...
from torch import device as Device, dtype as DType, cat, zeros, ones
...
class LineBreakPatchEncoder(fl.Module):
    ...
    Args:
        ...
        patch_size (int): The size of each square patch. The image width and height should be 
    ...
    def __init__(
        ...
        self.linebreak = fl.Parameter(
            dims = (1, 1, self.out_channels),
            requires_grad = True,
            device = device,
            dtype = dtype
        )
    ...
    def forward(self, x):
        ...
        patched_x_linebreak = torch.cat((patched_x, linebreak_embedding), dim=2)
